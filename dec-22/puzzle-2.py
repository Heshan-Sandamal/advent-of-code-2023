    ...
    for br in new_bricks_set:
        if (br != brick and br.min_z > 1):
            if (get_minimum_z_index(br, br.min_z) != br.min_z):
                # if a particular block is fallen, increased failed count & also remove it's cells from all cells
                failed_count += 1
                all_cells = all_cells - set(br.cells)
    return failed_count
...
for brick in new_bricks_set:
    k = set(all_cells)
    all_cells = all_cells - set(brick.cells)
    all_failed += try_fall_without_particular_brick(brick)
    all_cells = k

print("All Failed Count", all_failed)
